        ...
        public static int GetUserName(AMX amx1, AMXArgumentList args1, Script caller_script)
        {
            ...
            try
            {
                var res = Task.Run(async () =>
                {
                    ...
                    return caller_script.m_Instance.m_Bot.m_Client.GetChatMemberAsync(_Chat.Id, Convert.ToInt64(args1[1].AsString())).Result.User.Username;
                });
            ...
            catch (Exception ex)
            {
                ...
                Utils.Log.Error("In native 'GetUserName' (dest_string must be an char array, user not found, or invalid parameters!)" + caller_script);
            }
            return 1;
        }
    }
